...
export function RecommendationSection(props: SectionProps) {
      ...
      return (
        <div className="flex flex-col items-center gap-4 rounded-3xl border border-pink-100/50 bg-white/60 p-8 text-center shadow-sm dark:border-pink-500/20 dark:bg-black/20">
          ...
          {onRetry ? (
            <button
              onClick={onRetry}
              className="rounded-full bg-pink-200 px-5 py-2 text-sm font-medium text-pink-900 transition hover:bg-pink-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-pink-400 focus-visible:ring-offset-2"
            >
              Retry
            </button>
          ) : null}
    ...
    if (state === "empty") {
      return (
        <div className="flex flex-col items-center gap-4 rounded-3xl border border-dashed border-pink-200/70 bg-white/60 p-10 text-center shadow-sm dark:border-pink-500/30 dark:bg-black/10">
          ...
          <Link
            href="/products"
            className="rounded-full bg-pink-200 px-6 py-2 text-sm font-semibold text-pink-900 transition hover:bg-pink-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-pink-400 focus-visible:ring-offset-2"
          >
            Browse products
          </Link>
        </div>
  ...
  return (
    <section
      className={`rounded-[1.75rem] ${sectionGradient} px-6 py-8 shadow-[0_18px_48px_-24px_rgba(110,20,70,0.35)] lg:px-10 lg:py-10`}
...
export function ForYouSection() {
  const trpc = useTRPC();
  const options = trpc.recommendation.forYou.queryOptions({
    limit: 8,
...
export function TrendingSection() {
  const trpc = useTRPC();
  const options = trpc.recommendation.trending.queryOptions({
    limit: 8,
  }) as unknown as UseSuspenseQueryOptions<
    RecommendationEntry[],
    Error,
...
export function SimilarItemsSection(props: {
  productId: string;
  title?: string;
}) {
  const trpc = useTRPC();
  const options = props.productId
    ? trpc.recommendation.similar.queryOptions({
  ...
  const { data, refetch } = useSuspenseQuery(
    options as unknown as UseSuspenseQueryOptions<
      RecommendationEntry[],
      Error,
      RecommendationEntry[],
      readonly unknown[]
