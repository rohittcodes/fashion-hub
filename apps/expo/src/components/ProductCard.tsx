...
import { Alert, Pressable, Text, View, Platform, ToastAndroid } from "react-native";
...
export function ProductCard({ product }: ProductCardProps) {
  ...
  const discountPercentage = hasDiscount 
    ? Math.round(((parseFloat(product.compareAtPrice ?? "0") - parseFloat(product.price)) / parseFloat(product.compareAtPrice ?? "1")) * 100)
    : 0;
  ...
  const rawFirst = Array.isArray(product.images) ? (product.images[0] as unknown) : undefined;
  ...
  return (
    <View className="m-2 w-[45%] overflow-hidden rounded-2xl bg-white shadow-lg border border-gray-100">
      <Pressable onPress={() => router.push(`/products/${product.slug}`)}>
        <View className="relative aspect-square bg-gradient-to-br from-gray-50 to-gray-100">
          {hasDiscount && (
            <View className="absolute top-2 left-2 z-10 rounded-full bg-red-500 px-2 py-1">
              <Text className="text-xs font-bold text-white">-{discountPercentage}%</Text>
            </View>
          )}
          {product.isFeatured && (
            <View className="absolute top-2 left-20 z-10 rounded-full bg-pink-500 px-2 py-1">
          ...
          <ExpoImage
            ...
            onError={(e: unknown) => console.log("Image load error:", e, imageUrl)}
          />
          <View className="absolute bottom-2 right-2 z-10">
            <Pressable
              ...
              className={`rounded-full p-3 shadow-md ${
                addToCart.isPending || product.inventory === 0 ? "bg-gray-200" : "bg-pink-500"
              }`}
            >
              <Ionicons
                name="cart-outline"
                size={18}
                color={addToCart.isPending || product.inventory === 0 ? "#6B7280" : "#FFFFFF"}
              />
            </Pressable>
          </View>
      ...
      <View className="p-4">
        ...
        <Pressable onPress={() => router.push(`/products/${product.slug}`)}>
          <Text className="mt-1 text-base font-bold text-gray-900" numberOfLines={2}>
        ...
        {product.inventory > 0 && product.inventory < 10 && (
          <View className="mt-2 rounded-lg bg-orange-50 px-2 py-1">
            <Text className="text-xs text-orange-600 font-medium text-center">
